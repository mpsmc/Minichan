...
if(isset($_GET['no'])) {
	if(!is_numeric($_GET['no'])) {
		$_SESSION['notice'] = "Invalid bulletin."; exit(header("Location: ".DOMAIN."moderate_bulletins"));
		}
}
...
if(isset($_GET['mode'])) {
	if($_GET['mode'] != "allow") {
		if($_GET['mode'] != "deny") {
			$_SESSION['notice'] = "Method must either be allow or deny."; exit(header("Location: ".DOMAIN."moderate_bulletins"));
		}
	}
}
...
function allow_deny() {
	...
	if(isset($_GET['no']) && isset($_GET['mode'])) {
		...
		if($_GET['mode'] == "deny") {
			...
			if(!$send = mysql_query($sql)) {
				$_SESSION['notice'] = mysql_error(); exit(header("Location: ".DOMAIN."moderate_bulletins"));
				} else {
				$_SESSION['notice']="Bulletin denied"; exit(header("Location: ".DOMAIN."moderate_bulletins"));
				}
		}
	...
	$_SESSION['notice'] = "Error try again."; exit(header("Location: ".DOMAIN."moderate_bulletins"));
	}
}
