...
//				echo ' <a style="vertical-align: bottom;text-decoration: none;" title="Gold member" href="'.DOMAIN.'gold_account"><img style="border: none" src="'.DOMAIN.'style/star.png" /></a>';	
    ...
    if ($topic_flag != null) {
        echo ' <img style="border: none; vertical-align:middle" src="'.DOMAIN.'flags/'.$topic_flag.'.png" />';
    }
...
if (allowed('open_profile')) {
    echo '<li><a class="topic_profile_link" href="'.DOMAIN.'profile/'.$topic_author.'">Profile</a></li>';
}
if (allowed('mod_pm')) {
    echo '<li><a href="'.DOMAIN.'compose_message/'.$topic_author.'">PM</a></li>';
}
if (!$sticky && allowed('stick_topic')) {
    echo '<li><a href="'.DOMAIN.'stick_topic/'.$_GET['id'].'" onclick="return submitDummyForm(\''.DOMAIN.'stick_topic/'.$_GET['id'].'\', \'id\', '.$_GET['id'].', \'Stick this topic?\');">Stick</a></li>';
}
if ($sticky && allowed('stick_topic')) {
    echo '<li><a href="'.DOMAIN.'unstick_topic/'.$_GET['id'].'" onclick="return submitDummyForm(\''.DOMAIN.'unstick_topic/'.$_GET['id'].'\', \'id\', '.$_GET['id'].', \'Unstick this topic?\');">Unstick</a></li>';
}
if (!$locked && allowed('lock_topic')) {
    echo '<li><a href="'.DOMAIN.'lock_topic/'.$_GET['id'].'" onclick="return submitDummyForm(\''.DOMAIN.'lock_topic/'.$_GET['id'].'\', \'id\', '.$_GET['id'].', \'Really lock this topic?\');">Lock</a></li>';
}
if ($locked && allowed('lock_topic')) {
    echo '<li><a href="'.DOMAIN.'unlock_topic/'.$_GET['id'].'" onclick="return submitDummyForm(\''.DOMAIN.'unlock_topic/'.$_GET['id'].'\', \'id\', '.$_GET['id'].', \'Unlock this topic?\');">Unlock</a></li>';
}
if (!$deleted) {
    if (allowed('delete')) {
        echo '<li><a href="'.DOMAIN.'delete_topic/'.$_GET['id'].'" onclick="return submitDummyForm(\''.DOMAIN.'delete_topic/'.$_GET['id'].'\', \'id\', '.$_GET['id'].', \'Really delete this topic?\');">Delete</a></li>';
    }
    if (!$topic_stealth_banned && allowed('delete') && allowed('undelete')) {
        echo '<li><a href="'.DOMAIN.'stealth_delete_topic/'.$_GET['id'].'" onclick="return submitDummyForm(\''.DOMAIN.'stealth_delete_topic/'.$_GET['id'].'\', \'id\', '.$_GET['id'].', \'Really stealth delete this topic?\');">Stealth</a></li>';
    }
} else {
    if (allowed('undelete')) {
        echo '<li><a href="'.DOMAIN.'undelete_topic/'.$_GET['id'].'" onclick="return submitDummyForm(\''.DOMAIN.'undelete_topic/'.$_GET['id'].'\', \'id\', '.$_GET['id'].', \'Really undelete this topic?\');">Undelete</a></li>';
    }
}
if (allowed('delete_image') && ($topic_image_name || ($topic_img_external && $topic_thumb_external))) {
    echo '<li><a href="'.DOMAIN.'delete_image/topic/'.$_GET['id'].'" onclick="return submitDummyForm(\''.DOMAIN.'delete_image/topic/'.$_GET['id'].'\', \'id\', '.$_GET['id'].', \'Really delete this image?\');">Delete Image</a></li>';
}
if (allowed('set_image') && !($topic_image_name || ($topic_img_external && $topic_thumb_external))) {
    echo '<li><a href="'.DOMAIN.'set_image/topic/'.$_GET['id'].'" onclick="return chooseImage(this);">Set Image</a></li>';
}
if (allowed('set_time')) {
    echo '<li><a href="'.DOMAIN.'action.php?action=set_time&id='.$_GET['id'].'" onclick="return submitSetTime(this);">Bump</a></li>';
}
if (allowed('delete') || allowed('undelete') || allowed('delete_image') || allowed('lock_topic') || allowed('set_time') || allowed('stick_topic') || allowed('open_profile') || allowed('mod_pm')) {
...
//echo '<li><a class="addthis_button" href="http://www.addthis.com/bookmark.php?v=250&pubid=ra-4db55d8a51bfcd4c"><span>Share</span></a></li>';
    ...
    if (!$locked || allowed('lock_topic')) {
        echo '<li><a href="'.DOMAIN.'new_reply/'.$_GET['id'].'/quote_topic" onclick="quickQuote(\'OP\', \''.$reply_body_quickquote.'\');return false;">Quote</a></li>';
    }
...
function preg_replace_anchors($data)
{
    ...
    if (!array_key_exists($pure_id, $reply_ids)) {
        ...
        $hidden = '';
        if (in_array($pure_id, $hidden_replies)) {
            $hidden = ', hidden';
        }
...
while (fetchReplyList()) {
    ...
    if ($reply_deleted && !allowed('undelete')) {
        continue;
    }
    if ($reply_stealth && !allowed('undelete') && !canSeeStealthBannedPost($reply_author, $reply_ip)) {
        continue;
    }
    ...
    //				$out['author'] .= ' <a style="vertical-align: bottom;text-decoration: none;" title="Gold member" href="'.DOMAIN.'gold_account"><img style="border: none" src="'.DOMAIN.'style/star.png" /></a>';
        ...
        if ($reply_flag != null) {
            $out['author'] .= ' <img style="border: none; vertical-align:middle" src="'.DOMAIN.'flags/'.$reply_flag.'.png" />';
        }
    }
    ...
    } elseif ($reply_author == $previous_poster && $posts_in_row >= 17) {
        ++$posts_in_row;
        if (!MOBILE_MODE) {
            $out['action'] .=  'just kept on posting';
        }
    } else {
        $posts_in_row = 0;
        if (!MOBILE_MODE) {
    ...
    if (allowed('open_profile')) {
        echo '<li><a class="topic_profile_link" href="'.DOMAIN.'profile/'.$reply_author.'">Profile</a></li>';
    }
    if (allowed('mod_pm')) {
        echo '<li><a href="'.DOMAIN.'compose_message/'.$reply_author.'">PM</a></li>';
    }

    if (!$reply_deleted) {
        if (allowed('delete')) {
            echo '<li><a href="'.DOMAIN.'delete_reply/'.$reply_id.'" onclick="return submitDummyForm(\''.DOMAIN.'delete_reply/'.$reply_id.'\', \'id\', '.$reply_id.', \'Really delete this reply?\');">Delete</a></li>';
        }
        if (!$reply_stealth && allowed('delete') && allowed('undelete')) {
            echo '<li><a href="'.DOMAIN.'stealth_delete_reply/'.$reply_id.'" onclick="return submitDummyForm(\''.DOMAIN.'stealth_delete_reply/'.$reply_id.'\', \'id\', '.$reply_id.', \'Really stealth delete this reply?\');">Stealth</a></li>';
        }
    } else {
        if (allowed('undelete')) {
            echo '<li><a href="'.DOMAIN.'undelete_reply/'.$reply_id.'" onclick="return submitDummyForm(\''.DOMAIN.'undelete_reply/'.$reply_id.'\', \'id\', '.$reply_id.', \'Really undelete this reply?\');">Undelete</a></li>';
        }
    }

    if ((allowed('delete_image')) && ($reply_image_name || $img_external)) {
        echo '<li><a href="'.DOMAIN.'delete_image/topic/'.$_GET['id'].'/reply/'.$reply_id.'" onclick="return submitDummyForm(\''.DOMAIN.'delete_image/topic/'.$_GET['id'].'/reply/'.$reply_id.'\', \'id\', '.$reply_id.', \'Really delete this image?\');">Delete Image</a></li>';
    }
    if ((allowed('set_image')) && !($reply_image_name || $img_external)) {
        echo '<li><a href="'.DOMAIN.'set_image/topic/'.$_GET['id'].'/reply/'.$reply_id.'" onclick="return chooseImage(this);">Set Image</a></li>';
    }

    if (allowed('delete') || allowed('undelete') || allowed('delete_image') || allowed('open_profile') || allowed('mod_pm')) {
        echo '<li><strong>|</strong></li>';
    }
    ...
    if (!$locked || allowed('lock_topic')) {
        echo '<li><a href="'.DOMAIN.'new_reply/'.$_GET['id'].'/quote_reply/'.$reply_id.'" onclick="quickQuote('.$reply_id.', \''.$reply_body_quickquote.'\');return false;">Quote</a></li><li><a href="'.DOMAIN.'new_reply/'.$_GET['id'].'/cite_reply/'.$reply_id.'" onclick="quickCite('.$reply_id.');return false;">Cite</a></li>';
    }
...
<div id="quick_reply" class="noscreen">
    ...
    ?>
			<label for="post_hyperlink">Hyperlink:</label> <input class="inline" type="checkbox" name="post_hyperlink" id="post_hyperlink" <?php if ($administrator) {
