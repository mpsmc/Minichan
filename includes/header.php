...
if (substr($_SERVER['SERVER_ADDR'], 0, 7) == '::ffff:') {
    $_SERVER['SERVER_ADDR'] = substr($_SERVER['SERVER_ADDR'], 7);
}
...
if (empty($_COOKIE['UID']) || empty($_COOKIE['password'])) {
    ...
    if ($proxy) {
        if (!$_SESSION['proxy_notice']) {
            $_SESSION['notice'] = "Proxies are blocked from creating accounts. Please visit the site without using a proxy first or <a href='".DOMAIN."restore_ID'>restore your ID</a>.";
        }
...
function performBanCheck($table, $column, $value, $template)
{
    ...
    if ($link->num_rows($check) == 0 && !defined('TEST_BAN')) {
        return;
    }
...
if (!$user_settings) {
    $user_settings = dashboardDefaults();
}
