...
// Check if banned.
$check_uid_ban = $link->db_exec('SELECT filed, expiry, reason, stealth FROM uid_bans WHERE uid = %1', $_GET['uid']);
list($ban_filed, $ban_expiry, $banreason, $is_stealth_banned) = $link->fetch_row($check_uid_ban);
$banned = false;
if (!empty($ban_filed)) {
...
<ul class="menu">
<?php
if (!allowed('manage_reporting', $_GET['uid']) && allowed('manage_reporting') && $allow_user_report) {
    echo '<li><a onclick="return submitDummyForm(\''.DOMAIN.'disable_reporting/'.$_GET['uid'].'\', \'id\', \''.$_GET['uid'].'\', \'Really disallow this user from reporting?\');" href="'.DOMAIN.'disable_reporting/'.$_GET['uid'].'">Disallow reporting</a></li>';
}

if (!allowed('manage_reporting', $_GET['uid']) && allowed('manage_reporting') && !$allow_user_report) {
    echo '<li><a onclick="return submitDummyForm(\''.DOMAIN.'enable_reporting/'.$_GET['uid'].'\', \'id\', \''.$_GET['uid'].'\', \'Really allow this user to report?\');" href="'.DOMAIN.'enable_reporting/'.$_GET['uid'].'">Allow reporting</a></li>';
}

if ($banned && allowed('ban_uid')) {
    echo '<li><a onclick="return submitDummyForm(\''.DOMAIN.'unban_poster/'.$_GET['uid'].'\', \'id\', \''.$_GET['uid'].'\', \'Really unban this poster?\');" href="'.DOMAIN.'unban_poster/'.$_GET['uid'].'">Unban</a></li>';
}
...
*/
if ($administrator) {
    echo '<li><a href="'.DOMAIN.'nuke_ID/'.$_GET['uid'].'" onclick="return submitDummyForm(\''.DOMAIN.'nuke_ID/'.$_GET['uid'].'\', \'id\', \''.$_GET['uid'].'\', \'Really delete all topics and replies by this poster?\');">Delete all posts</a></li>';
}
