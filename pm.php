...
if ( $source != $_SESSION['UID'] && $destination != $_SESSION['UID'] ) $error = true;
if ( ( $source == 'mods' || $destination == 'mods' ) && ( allowed("mod_pm") ) ) $error = false;
if ( ( $source == 'admins' || $destination == 'admins' ) && $administrator ) $error = false;
...
if(!$read) {
	if($_SESSION['num_pms']>0) $_SESSION['last_pm_check'] = 0;
...
if($source == $_SESSION['UID']) {
	...
	if(allowed("open_profile")) echo '<a href="'.DOMAIN.'profile/'.$source.'">';
	echo modname($source);
	if(allowed("open_profile")) echo '</a>';
}
...
if($destination == $_SESSION['UID']) {
	...
	if(allowed("open_profile")) echo '<a href="'.DOMAIN.'profile/'.$destination.'">';
