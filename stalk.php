...
function search_uid($uid)
{
    ...
    while (list($ip) = $link->fetch_row($search)) {
        if (in_array(array($uid, $ip), $data['connections']) || in_array(array($ip, $uid), $data['connections'])) {
            continue;
        }
...
function search_ip($ip)
{
    ...
    while (list($uid) = $link->fetch_row($search)) {
        if (in_array(array($ip, $uid), $data['connections']) || in_array(array($uid, $ip), $data['connections'])) {
            continue;
        }
