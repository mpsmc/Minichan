...
$link->db_exec("SELECT count(mod_UID) as amount, mod_UID FROM mod_actions WHERE action not in ('open_ip', 'open_profile') AND time > UNIX_TIMESTAMP() - 3600 * 24 * 31 GROUP BY mod_UID ORDER BY amount DESC");
while (list($count, $uid) = $link->fetch_row()) {
    $modname = modname($uid);
    if ($modname == 'a user') {
        continue;
    }
...
if (is_array($label)) {
    for ($i = 0; $i < count($label); ++$i) {
        ...
        } elseif ($label_form == 'square' && $show_label && $data[$i] > 0) {
