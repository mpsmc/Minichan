...
<form action="" method="post">
	...
	<div class="row">
		...
		<input type="checkbox" value="1" name="history" id="search_history" class="inline" <?php if($_GET['history']==1) { ?>checked="checked" <?php } ?> /> <label class="inline" for="search_history">Your history only</label><br />
	</div>
</form>
<?php
if (!empty($_GET['q'])) {
...
//var_dump($search_replies);die();
	...
	if(!$erred) $analytics_track_url = "/search?q=".$_GET['q']."&category=".(($_GET['deep_search']) ? 'deep' : 'quick');
}
print_errors();
