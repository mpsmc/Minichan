...
if ($_POST['form_sent']) {
    ...
    if (!$erred) {
        ...
        foreach ($_POST['form'] as $key => $value) {
            ...
            if ($key == 'memorable_password' && !$value) {
                continue;
            }
...
<form action="" method="post">
    ...
    echo ' selected';
			...
			<option value="1"<?php if ($user_config['topics_mode'] == 1) {
    echo ' selected';
} ?>>Date created</option>
		</select>
	</div>
	<div class="row">
        ...
        if (!$user_config['style']) {
            $user_config['style'] = DEFAULT_STYLESHEET;
        }
        foreach ($_AVAILABLE_STYLES as $style) {
            if (!file_exists(SITE_ROOT.'/style/'.$style.'.css') && $style != 'Custom') {
                continue;
            }
    ...
    echo ' selected';
        ...
        <option value="0"<?php if ($user_config['image_viewer'] == 0) {
    echo ' selected';
} ?>>Off (Current tab)</option>
		<option value="2"<?php if ($user_config['image_viewer'] == 2) {
    echo ' selected';
} ?>>Off (New tab)</option>
        </select>
    </div>
    ...
    echo ' selected';
			...
			<option value="100"<?php if ($user_config['snippet_length'] == 100) {
    echo ' selected';
} ?>>100</option>
			<option value="120"<?php if ($user_config['snippet_length'] == 120) {
    echo ' selected';
} ?>>120</option>
			<option value="140"<?php if ($user_config['snippet_length'] == 140) {
    echo ' selected';
} ?>>140</option>
			<option value="160"<?php if ($user_config['snippet_length'] == 160) {
    echo ' selected';
} ?>>160</option>
		</select>
		<p class="caption"></p>
