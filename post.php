...
if ($_POST['form_sent']) {
    // CSRF checking.
    check_token();
    if (function_exists('private_botDetect')) {
        private_botDetect();
    }
    $token = $_GET['token'];
    ...
    if ((USER_REPLIES < RECAPTCHA_MIN_POSTS) && (recaptcha_valid() !== true)) {
        add_error('Please fill out the captcha.');
    }
    ...
    if ($_POST['post']) {
        ...
        if (!((ALLOW_IMAGES || ALLOW_IMGUR) && (!empty($_FILES['image']['name']) || !empty($_POST['imageurl'])))) {
        ...
        if (ALLOW_IMAGES && !empty($_FILES['image']['name']) && !$editing) {
            ...
            if ($uploading) {
                ...
                } elseif (!in_array($image_data['type'], $valid_types)) {
        ...
        } elseif (ALLOW_IMGUR && !$editing && $_POST['imageurl']) {
            ...
            if ($locked != 0 && !allowed('lock_topic')) {
                ...
                if (!$stealth_banned) {
                    ...
                    if ($inserted_id) {
                        ...
                        while ($row = $link->fetch_row()) {
                            if (is_array($citations_sent) && in_array($row[0], $citations_sent)) {
                                continue;
                            }

                            if (in_array($row[0], $watchlists_sent)) {
                                continue;
                            }
        ...
        if (!$erred) {
            ...
            if ($uploading) {
                ...
                if ($previous_image && file_exists('img/'.$previous_image) && file_exists('thumbs/'.$previous_image)) {
                    ...
                    if (file_exists('img/'.$previous_image) && $previous_image) {
                        @unlink('img/'.$previous_image);
                    }
                    if (file_exists('thumbs/'.$previous_image) && $previous_image) {
                        @unlink('thumbs/'.$previous_image);
                    }
...
?>
<form action="<?php echo $url; ?>" method="post"<?php if (ALLOW_IMAGES) {
...
<?php 
        ...
        } elseif (isset($_GET['quote_topic']) || ctype_digit($_GET['quote_reply'])) { // Otherwise, fetch any text we may be quoting.
            // Fetch the topic.
...
} elseif ($reply && !isset($_GET['quote_topic']) && !isset($_GET['quote_reply']) && !$editing) { // If we're not citing or quoting, display the original post.
    $stmt = $link->db_exec('SELECT body FROM topics WHERE id = %1', $_GET['reply']);
