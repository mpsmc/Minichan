...
// Input sanitation
if (!in_array($search_for, array('topics', 'replies', 'both'))) {
    $search_for = null;
}
if (strlen($topic_headline) > 100) {
    $topic_headline = substr($topic_headline, 0, 100);
}
if (strlen($author_name) > 30) {
    $author_name = substr($author_name, 0, 30);
}
if (!in_array($sort_by, array('age', 'headline_relevance', 'body_relevance', 'combined_relevance'))) {
    $sort_by = null;
}
...
<tr><td class="minimal"><label for="search_for">Search for:</label></td><td><select name="search_for" id="search_for">
	<option value="both" <?php if ($search_for == 'both') {
    echo "selected='selected' ";
} ?>>Topics & Replies</option>
	<option value="topics" <?php if ($search_for == 'topics') {
    echo "selected='selected' ";
} ?>>Topics</option>
	<option value="replies" <?php if ($search_for == 'replies') {
    echo "selected='selected' ";
} ?>>Replies</option>
</select></td></tr>
...
<?php if (allowed('open_profile')) {
    ?>
<tr><td class="minimal"><label for="profile">Profile:</label></td><td><input type="text" name="profile" id="profile" size="30" value="<?php echo htmlspecialchars($profile);
    ?>" /></td></tr>
<?php
...
if (allowed('open_ip')) {
    ?>
<tr><td class="minimal"><label for="IP_address">IP address:</label></td><td><input type="text" name="IP_address" id="IP_address" size="30" value="<?php echo htmlspecialchars($IP_address);
    ?>" /></td></tr>
<?php 
} ?>
<tr><td class="minimal"><label for="my_history">My history only:</label></td><td><input type="checkbox" name="my_history" id="my_history" <?php if ($my_history) {
    echo "checked='checked'";
} ?> /></td></tr>
<tr><td class="minimal"><label for="only_images">With image only:</label></td><td><input type="checkbox" name="only_images" id="only_images" <?php if ($only_images) {
    echo "checked='checked'";
} ?> /></td></tr>
<tr><td class="minimal"><label for="hide_images">Hide images from result:</label></td><td><input type="checkbox" name="hide_images" id="hide_images" <?php if ($hide_images) {
    echo "checked='checked'";
} ?> /></td></tr>
...
<tr><td class="minimal"><label for="sort_by">Sort results by:</label></td><td><select name="sort_by" id="sort_by">
	<option value="age" <?php if ($sort_by == 'age') {
    echo "selected='selected' ";
} ?>>Age</option>
	<option value="headline_relevance" <?php if ($sort_by == 'headline_relevance') {
    echo "selected='selected' ";
} ?>>Headline relevance</option>
	<option value="body_relevance" <?php if ($sort_by == 'body_relevance') {
    echo "selected='selected' ";
} ?>>Body relevance</option>
	<option value="combined_relevance" <?php if ($sort_by == 'combined_relevance') {
    echo "selected='selected' ";
} ?>>Combined relevance</option>
</select> <input type="checkbox" name="ascending" id="ascending" <?php if ($ascending) {
    echo 'checked="checked"';
} ?>> <label for="ascending">Ascending</label></td></tr>
...
if ($search) {
    ...
    if ($author_name && mb_ereg('^(.*?)(!!?[^!]+)?$', $author_name, $regs)) {
        ...
        if ($trip) {
            $conditions['author_trip'] = "t1.tripfag = ' ".$link->escape($trip)."'";
        } // Whitespace is intentional (old bug persistant in db)
    }
    ...
    } elseif ($search_topics && !$search_replies) {
        $query = $query_topics.' ';
    } elseif ($search_replies && !$search_topics) {
        $query = $query_replies.' ';
    } else {
    ...
    } elseif ($sort_by == 'headline_relevance' && $conditions['topic_headline']) {
        $query .= 'ORDER BY '.$conditions['topic_headline'].$order;
    } elseif ($sort_by == 'body_relevance' && $conditions['message_body']) {
        $query .= 'ORDER BY '.$conditions['message_body'].$order;
    } elseif ($sort_by == 'combined_relevance') {
        $sorts = array();
    ...
    if ($num_rows > 0) {
        while (list($id, $parent_id, $thumb_width, $thumb_height, $image_name, $img_external, $thumb_external, $author, $author_ip, $name, $trip, $poster_number, $time, $replies, $visits, $headline, $body, $is_stealth_banned) = $link->fetch_row($query)) {
            if ($is_stealth_banned && !canSeeStealthBannedPost($author, $author_ip)) {
                continue;
            }
            echo '<h3>';
            if (!$name && !$trip) {
