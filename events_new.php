...
if(!allowed("manage_events")) {
	...
	$_SESSION['notice']="The posting of events by users has currently been disabled."; header("Location: ".DOMAIN.""); exit("");
	}
}
...
if(!allowed("manage_events")) {
	...
	$total = $num_t + $num_r;
		...
		echo '<p>You can not post events at this time.</p>'; require 'includes/footer.php'; exit("");
		}
}
...
function post_event($id=-1) {
	...
	if(!$des) {
	$_SESSION['notice'] = "No description"; exit(header("Location: ".DOMAIN."new_event"));
	}
		if(strlen($des)>256) {
	...
	if(empty($date)) {
	$_SESSION['notice'] = "Date format incorrect."; exit(header("Location: ".DOMAIN."new_event"));
	}
	...
	$date = explode("-",$date);
	if($date[1] > 12) {
	$_SESSION['notice'] = "Invalid month.";  exit(header("Location: ".DOMAIN."new_event"));
	}
	if($date[0] < date("Y")) {
	$_SESSION['notice'] = "Date can not be set in the past."; exit(header("Location: ".DOMAIN."new_event"));
	}
	if($date[2] > 31) {
	$_SESSION['notice'] = "Invalid day."; exit(header("Location: ".DOMAIN."new_event"));
	}
	...
	$_SESSION['notice'] = "Invalid date."; exit(header("Location: ".DOMAIN."new_event"));
	} 
	
	// Checking for leap years.
	if($date[1] == "02") {
		if($date[2] == "29" && !in_array($date[0],$years)) {
		$_SESSION['notice'] = "It's not a leap year.";  exit(header("Location: ".DOMAIN."new_event"));
		} 
	}
