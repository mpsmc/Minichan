...
if (!allowed('manage_bulletins')) {
    if (!ALLOW_USER_BULLETINS) {
        $_SESSION['notice'] = 'The posting of bulletins by users has currently been disabled';
...
function display_new_bulletin($error, $text = '', $id = -1)
{
    ...
    if (!allowed('manage_bulletins')) {
        if (!ALLOW_USER_BULLETINS) {
            echo '<p>You can not post bulletins at this time.</p>';
            require 'includes/footer.php';
            exit('');
        }
    }
...
if (!allowed('manage_bulletins')) {
    ...
    if (!allowed('manage_bulletins')) {
        ...
        while ($get = mysql_fetch_array($send)) {
            $time = $get['time'];
            $diff = time() - $time;
            if ($diff < $time_between_bulletins) {
